        ...
        for env in configs.eval_sets[dataset]:
            ...
            for shot in [1, 4, 8, 16, 32]:
                ...
                if not osp.exists(osp.join(configs.ckpt_path, dataset, env, f"{shot}-shots", ckpt[:-4], "best.pth")):
                    ...
                    linDataset = MultiEnvDataset(osp.join(f"/data/chengengfeng/CLAP/data/fewshot_datasets/{shot}-shots", dataset), test_env=env, transform=preprocess)
                    dataloader = tud.DataLoader(linDataset, batch_size=32, shuffle=True, num_workers=4, drop_last=False)
